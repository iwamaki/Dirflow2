...
class UIController {
    ...
    static async applyChanges() {
        ...
        try {
            ...
            if (fileIndex !== -1) {
                ...
                MessageHandler.addMessage('system', `✅ "${AppState.currentEditingFile}" に差分を適用しました`);
            ...
            MessageHandler.addMessage('system', `❌ 差分適用エラー: ${error.message}`);
        }
    }
    ...
    static async applySelectedChanges() {
        ...
        if (selectedCount === 0) {
            MessageHandler.addMessage('system', '⚠️ 適用する変更を選択してください');
            return;
        }
        ...
        if (newContent === null) {
            MessageHandler.addMessage('system', '❌ 内容の生成に失敗しました');
            return;
        }
        ...
        try {
            ...
            if (fileIndex !== -1) {
                ...
                MessageHandler.addMessage('system', `✅ "${AppState.currentEditingFile}" に選択された変更 (${selectedCount}件) を適用しました`);
