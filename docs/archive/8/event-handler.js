...
import { elements, mockFileSystem } from './public/src/config.js';
import { AppState, SystemPromptManager } from './state.js';
import { Utils } from './utils.js';
import { UIController, FileManager } from './ui-controller.js';
...
export class EventHandler {
    static init() {
        ...
        elements.drawerCloseBtn.addEventListener('click', () => UIController.togglePromptDrawer(false));
        elements.drawerOverlay.addEventListener('click', () => UIController.togglePromptDrawer(false));
        ...
        document.querySelectorAll('.drawer-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                if (section) {
                    UIController.switchPromptSection(section);
                }
            });
        });
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                ...
                if (AppState.isChatOpen) UIController.toggleChat();
                if (AppState.isPromptDrawerOpen) UIController.togglePromptDrawer(false);
            }
        });
    }
    ...
    static togglePromptDrawer() {
        UIController.togglePromptDrawer();
    }

    // 保存ボタンクリック処理
    static handleSaveClick() {
        if (!AppState.isContentModified) {
            MessageHandler.addMessage('system', '💡 変更点はありません。');
            return;
        }
        ...
        if (textarea && AppState.originalContent !== null) {
            ...
            MessageHandler.addMessage('system', '📊 保存する内容の差分を確認してください');
        } else {
    ...
    static toggleEditMode() {
        ...
        if (AppState.isDiffMode) {
            ...
            const contentToShow = EventHandler.currentEditingContent || EventHandler.getOriginalFileContent();
            FileManager.showFileContent(contentToShow, AppState.currentEditingFile);
        ...
        if (newEditMode) {
            ...
            contentToShow = EventHandler.currentEditingContent || EventHandler.getOriginalFileContent();
        } else {
            ...
            if (textarea) {
                ...
                EventHandler.currentEditingContent = textarea.value;
            } else {
                contentToShow = EventHandler.currentEditingContent || EventHandler.getOriginalFileContent();
            }
        }
    ...
    static async handleSystemPrompt() {
        ...
        try {
            ...
            if (isEditing) {
                ...
                MessageHandler.addMessage('system', `🧠 システムプロンプト "${name}" を更新しました`);
            ...
            if (AppState.currentPromptSection === 'manage') {
                UIController.refreshPromptList();
            }

        } catch (error) {
            MessageHandler.addMessage('system', `❌ プロンプト${originalText === '更新' ? '更新' : '登録'}に失敗しました: ${error.message}`);
        } finally {
